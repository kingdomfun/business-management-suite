<script lang="ts">
  import { state as appState, addEvent, updateEvent, removeEvent } from "../lib/state";
  ...
  function add() {
    addEvent({ date: defaultDate || dateKey(new Date()), time: "09:00", label: "" });
  }
</script>
  ...
  <div class="cblock">
    <div class="crow">
      ...
      <input
        ...
        value={e.time}
        onchange={(ev) => updateEvent(e.id, { time: ev.currentTarget.value })}
      />
      <button class="sq" title="Remove appointment" aria-label="Remove appointment" onclick={() => removeEvent(e.id)}>✕</button>
    </div>
    <input
      type="text"
      aria-label="Appointment title"
      placeholder="What (e.g. Call client)"
      value={e.label}
      oninput={(ev) => updateEvent(e.id, { label: ev.currentTarget.value })}
    />
    <input
      type="text"
      ...
      value={e.detail ?? ""}
      oninput={(ev) => updateEvent(e.id, { detail: ev.currentTarget.value })}
    />
    <label class="chk">
      <input
        type="checkbox"
        checked={!!e.replacesDay}
        onchange={(ev) => updateEvent(e.id, { replacesDay: ev.currentTarget.checked })}
      />
      Replace the whole day's schedule
    </label>
  </div>
{/each}
